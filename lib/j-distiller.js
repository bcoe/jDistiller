...
jDistiller.prototype.distill = function(url, callback) {
	...
	if (this._isURL(url)) {
		getRawPage = function(url, callback) {
			_this.distillURL(url, callback);
		};
	}
	else if (Buffer.isBuffer(url)) {
		getRawPage = this.distillBuffer;
	}
	else if (typeof url === 'string') {
		getRawPage = this.distillString;
	}

	getRawPage(url, function(err, rawPage) {
		if (err) {
			callback(err, null);
			return;
		}
...
jDistiller.prototype.distillURL = function(url, callback) {
	this.request({
		...
		var error = err || (res.statusCode < 200 || res.statusCode >= 300) ? res.statusCode : false,
			page = null;
...
jDistiller.prototype._object = function(object, page, distilledPage) {
	...
	Object.keys(object).forEach(function(k) {
			distilledPage[k] = _this._parseValue(k, object[k], page);
	});
